...
function cleanFiles() {
  return axios.post(`${baseURL}/documents/reset`).then((response) => {
...
function getDocuments() {
  return axios.get(`${baseURL}/documents`).then((response) => {
    const result = response.data;
    return result;
  });
}
...
function createDocument(
  ...
  return axios
    .post(`${baseURL}/documents/create/${name}`, { userName: userName })
    .then((response) => {
      const result = response.data;
      return result;
    });
...
function getDocument(
  ...
  return axios.put(`${baseURL}/documents/${name}`, body).then((response) => {
    const result = response.data;
    return result;
  });
}
...
function clearFormula(
  ...
  return axios
    .put(`${baseURL}/document/clear/formula/${docName}`, body)
    .then((response) => {
      const result = response.data;
      return result;
    });
...
function addToken(
  ...
  return axios
    .put(`${baseURL}/document/addtoken/${docName}`, body)
    .then((response) => {
      const result = response.data;
      return result;
    });
...
function addCell(
  ...
  return axios
    .put(`${baseURL}/document/addcell/${docName}`, body)
    .then((response) => {
      const result = response.data;
...
function removeToken(
  ...
  return axios
    .put(`${baseURL}/document/removetoken/${docName}`, body)
    .then((response) => {
      const result = response.data;
      return result;
    });
...
function requestEditCell(
  ...
  return axios
    .put(`${baseURL}/document/cell/edit/${docName}`, body)
    .then((response) => {
      const result = response.data;
      return result;
    });
...
function requestViewCell(
  ...
  return axios
    .put(`${baseURL}/document/cell/view/${docName}`, body)
    .then((response) => {
      const result = response.data;
      return result;
    });
...
function checkFormulaAndDisplay(
  document: any,
  formula: string,
  result: string
) {
  ...
  if (formulaFound !== formula) {
    console.log(
      `FAILURE: formula is not ${formula}, found ${formulaFound} instead`
    );
    return false;
  } else {
  ...
  if (resultFound !== result) {
    console.log(
      `FAILURE: result is not ${result}, found ${resultFound} instead`
    );
    return false;
  } else {
...
function checkIsEditing(document: any, isEditing: boolean) {
  ...
  if (isEditingFound !== isEditing) {
    console.log(
      `FAILURE: isEditing is not ${isEditing}, found ${isEditingFound} instead`
    );
    return false;
  } else {
...
function checkCell(
  ...
  if (cellTransport.value !== value) {
    console.log(
      `FAILURE: cell ${cell} value is not ${value}, found ${cellTransport.value} instead`
    );
    return false;
  } else {
  ...
  if (cellTransport.formula.length !== formula.length) {
    console.log(
      `FAILURE: cell ${cell} formula length is not ${formula.length}, found ${cellTransport.formula.length} instead`
    );
    return false;
  } else {
    console.log(
      `SUCCESS: cell ${cell} formula length is ${formula.length}, this succeeded`
    );
  }
  const foundFormula = cellTransport.formula as string[];
  if (!checkFormula(formula, foundFormula)) {
    console.log(
      `FAILURE: cell ${cell} formula is not [${formula}], found [${cellTransport.formula}] instead`
    );
    return false;
  } else {
    console.log(
      `SUCCESS: cell ${cell} formula is [${formula}], this succeeded`
    );
  }
  if (cellTransport.error !== error) {
    console.log(
      `FAILURE: cell ${cell} error is not ${error}, found ${cellTransport.error} instead`
    );
    return false;
  } else {
...
async function runTests() {
  ...
  let resultDocument = await requestViewCell(
    testDocument1,
    cellA1,
    userJuancho
  );
